...
/* playGreatHall is located in dominion.c */
  int checkPlayGreatHall(int p, struct gameState *post) {
  
    /* prior state of affairs */
    struct gameState pre;
    memcpy (&pre, post, sizeof(struct gameState));
  
    int preNumActions = pre.numActions;
    playGreatHall(p,post,0);
  
    /* The assignment 2 bug was that numActions is decremented instead of incremented */
    int postNumActions = post->numActions;
    assert(preNumActions <= postNumActions);
